...
export async function getSalesOrderDetail(
  ...
  try {
    const { data: o, error: oErr } = await supabase
      .from('sales_orders')
      .select('id, code, status, order_date, warehouse_id, customers(id, code, name)')
      .eq('id', orderId)
      .maybeSingle();
    ...
    // Realized figures accrue as the order ships (full or partial); they describe
    // the dispatched portion, so realized margin ties out against realized revenue.
    const realized = o.status === 'shipped' || o.status === 'partially_shipped';
    const realizedRevenue = realized
      ? lines.reduce((s, l) => s + l.unitPrice * l.shippedQuantity, 0)
      : null;
    ...
    const customer = o.customers
      ? { id: o.customers.id, code: o.customers.code, name: o.customers.name }
      : null;
