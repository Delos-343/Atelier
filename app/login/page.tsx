...
export default function LoginPage() {
  ...
  const onCaptchaError = useCallback((reason: string) => setCaptchaError(reason), []);
  ...
  async function signIn() {
    ...
    if (captchaOn && !captchaToken) {
      setError(captchaError ?? 'Please complete the “I’m human” check.');
      return;
    }
    setBusy(true);
    setError(null);
  ...
  return (
    <main className="grid min-h-[72vh] place-items-center px-5 py-8">
      <div className="card w-full max-w-[380px] p-8">
        <h1 className="text-[1.6rem] font-semibold tracking-[0.01em]">TechnicoFlor</h1>
        ...
        {supabase && <TurnstileWidget ref={captchaRef} onToken={onCaptcha} onError={onCaptchaError} />}
        {captchaError && <p className="mt-2 text-[0.8rem] text-muted">{captchaError}</p>}
