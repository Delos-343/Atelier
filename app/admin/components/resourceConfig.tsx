...
export const RESOURCES: Record<string, ResourceConfig> = {
  ...
  customers: {
    ...
    columns: [
      ...
      { key: 'payment_terms_days', label: 'Terms', align: 'right', render: (v) => (v == null ? '—' : `Net ${v}`) },
      { key: 'discount_pct', label: 'Disc.', align: 'right', render: (v) => (v == null || Number(v) === 0 ? '—' : `${Number(v)}%`) },
      { key: 'tax_id', label: 'NPWP', render: dash },
      { key: 'tax_exempt', label: 'PPN', render: (v) => (v ? 'Exempt' : 'Taxable') },
    ],
    fields: [
      ...
      { name: 'payment_terms_days', label: 'Payment terms (net days)', type: 'number', step: '1', placeholder: '30' },
      { name: 'discount_pct', label: 'Standing discount (%)', type: 'number', step: '0.01', placeholder: '0' },
      { name: 'tax_id', label: 'NPWP (tax ID)', type: 'text', placeholder: '00.000.000.0-000.000' },
      { name: 'tax_exempt', label: 'PPN exempt (zero-rated)', type: 'checkbox' },
    ],
  },
  ...
  suppliers: {
    ...
    fields: [
      { name: 'code', label: 'Code', type: 'text', required: true, placeholder: 'SUP-AROMA' },
      { name: 'name', label: 'Name', type: 'text', required: true, placeholder: 'Aroma Source Ltd' },
      { name: 'email', label: 'Email', type: 'text', placeholder: 'sales@aroma.com' },
      { name: 'phone', label: 'Phone', type: 'text' },
      { name: 'address', label: 'Address', type: 'text' },
      { name: 'payment_terms_days', label: 'Payment terms (net days)', type: 'number', step: '1', placeholder: '30' },
      { name: 'tax_id', label: 'NPWP (tax ID)', type: 'text', placeholder: '00.000.000.0-000.000' },
