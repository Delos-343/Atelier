...
describe('sales order returns / credit notes', () => {
  const confirm = (orderId: string) =>
    q(`update sales_orders set status='confirmed' where id=$1`, [orderId]);
  const ship = (orderId: string) => q(`select ship_sales_order($1, null)`, [orderId]);
  const availLot = (productId: string, whId: string, code: string, qty: number, cost: number, expiry: string) =>
  ...
  const doReturn = (orderId: string, code: string, lines: { line_id: string; quantity: number }[]) =>
    q(`select create_return($1, $2, $3::jsonb, null)`, [orderId, code, JSON.stringify(lines)]);

  it('restocks at blended cost, reverses COGS, and records a credit note', async () => {
    const wh = await createWarehouse();
    const prod = await createProduct('P1', 'l');
    const cust = await customer();
    await availLot(prod, wh, 'FG-A', 6, 0.4, '2026-12-01');
    await availLot(prod, wh, 'FG-B', 10, 0.5, '2027-06-01');
    const orderId = await createOrder('SO-R1', cust, wh, [
      { product_id: prod, quantity: 10, unit: 'l', unit_price: 2.0 },
    ]);
    await confirm(orderId);
    await ship(orderId); // 6*0.40 + 4*0.50 → cogs 4.40, shipped 10
    const la = await lineId(orderId, 'P1');

    await doReturn(orderId, 'CN-R1', [{ line_id: la, quantity: 5 }]);

    const [line] = await q(
      `select shipped_quantity, returned_quantity, cogs from sales_order_lines where id=$1`,
      [la],
    );
    expect(Number(line.shipped_quantity)).toBeCloseTo(10, 6); // shipped unchanged by a return
    expect(Number(line.returned_quantity)).toBeCloseTo(5, 6);
    expect(Number(line.cogs)).toBeCloseTo(2.2, 6); // 4.40 − 5*(4.40/10)

    const [lot] = await q(
      `select quantity_on_hand, unit_cost, status from inventory_lots where lot_code like 'CN-R1-%'`,
    );
    expect(Number(lot.quantity_on_hand)).toBeCloseTo(5, 6); // back in stock
    expect(Number(lot.unit_cost)).toBeCloseTo(0.44, 6); // blended 4.40/10
    expect(lot.status).toBe('available');

    const [cn] = await q(`select id from credit_notes where code='CN-R1'`);
    expect(cn).toBeTruthy();
    const [cnl] = await q(
      `select quantity, unit_price, cogs_reversed from credit_note_lines where credit_note_id=$1`,
      [cn.id],
    );
    expect(Number(cnl.quantity)).toBeCloseTo(5, 6);
    expect(Number(cnl.unit_price)).toBeCloseTo(2.0, 6);
    expect(Number(cnl.cogs_reversed)).toBeCloseTo(2.2, 6);

    const [costed] = await q(`select realized_margin from sales_order_lines_costed($1)`, [orderId]);
    expect(Number(costed.realized_margin)).toBeCloseTo(7.8, 6); // 2.00*(10−5) − 2.20

    const [o] = await q(`select status from sales_orders where id=$1`, [orderId]);
    expect(o.status).toBe('shipped'); // status unchanged by the return

    const [m] = await q(
      `select count(*)::int as n, sum(quantity) as net from stock_movements where movement_type='return'`,
    );
    expect(m.n).toBe(1);
    expect(Number(m.net)).toBeCloseTo(5, 6); // positive re-entry
  });

  it('rejects returning more than is out with the customer', async () => {
    const wh = await createWarehouse();
    const prod = await createProduct('P1', 'l');
    const cust = await customer();
    await availLot(prod, wh, 'FG-A', 10, 0.4, '2026-12-01');
    const orderId = await createOrder('SO-R2', cust, wh, [
      { product_id: prod, quantity: 10, unit: 'l', unit_price: 2.0 },
    ]);
    await confirm(orderId);
    await ship(orderId);
    const la = await lineId(orderId, 'P1');
    await expect(doReturn(orderId, 'CN-R2', [{ line_id: la, quantity: 11 }])).rejects.toThrow();
    const [line] = await q(`select returned_quantity from sales_order_lines where id=$1`, [la]);
    expect(Number(line.returned_quantity)).toBe(0); // atomic — nothing returned
    const [c] = await q(`select count(*)::int as n from credit_notes where code='CN-R2'`);
    expect(c.n).toBe(0); // no credit note
  });

  it('accumulates across multiple returns and blocks exceeding net-out', async () => {
    const wh = await createWarehouse();
    const prod = await createProduct('P1', 'l');
    const cust = await customer();
    await availLot(prod, wh, 'FG-A', 10, 0.4, '2026-12-01');
    const orderId = await createOrder('SO-R3', cust, wh, [
      { product_id: prod, quantity: 10, unit: 'l', unit_price: 2.0 },
    ]);
    await confirm(orderId);
    await ship(orderId);
    ...
    await doReturn(orderId, 'CN-R3a', [{ line_id: la, quantity: 4 }]);
    await doReturn(orderId, 'CN-R3b', [{ line_id: la, quantity: 3 }]);
    let rows = await q(`select returned_quantity, cogs from sales_order_lines where id=$1`, [la]);
    expect(Number(rows[0].returned_quantity)).toBeCloseTo(7, 6);
    expect(Number(rows[0].cogs)).toBeCloseTo(1.2, 6); // 4.00 − 7*0.40

    await expect(doReturn(orderId, 'CN-R3c', [{ line_id: la, quantity: 4 }])).rejects.toThrow();
    rows = await q(`select returned_quantity from sales_order_lines where id=$1`, [la]);
    expect(Number(rows[0].returned_quantity)).toBeCloseTo(7, 6); // unchanged
    const [c] = await q(`select count(*)::int as n from credit_notes where sales_order_id=$1`, [orderId]);
    expect(c.n).toBe(2); // the third rolled back
  });

  it('allows a return against a partially shipped order without changing its status', async () => {
    const wh = await createWarehouse();
    const prod = await createProduct('P1', 'l');
    const cust = await customer();
    await availLot(prod, wh, 'FG-A', 6, 0.4, '2026-12-01'); // only 6 of 10 can ship
    const orderId = await createOrder('SO-R4', cust, wh, [
      { product_id: prod, quantity: 10, unit: 'l', unit_price: 2.0 },
    ]);
    await confirm(orderId);
    await ship(orderId); // partially_shipped, shipped 6
    const la = await lineId(orderId, 'P1');

    await doReturn(orderId, 'CN-R4', [{ line_id: la, quantity: 2 }]); // 2 of the 6 out
    const [line] = await q(
      `select shipped_quantity, returned_quantity from sales_order_lines where id=$1`,
      [la],
    );
    expect(Number(line.shipped_quantity)).toBeCloseTo(6, 6);
    expect(Number(line.returned_quantity)).toBeCloseTo(2, 6);
    const [o] = await q(`select status from sales_orders where id=$1`, [orderId]);
    expect(o.status).toBe('partially_shipped'); // unchanged
  });
});
